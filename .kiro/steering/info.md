...
**MCP Godot:**
- Доступен для прямого взаимодействия с Godot, ознакомься с функционалом
- Проект: `D:\AnimationApp\project.godot`
- Обязательно проводи хорошие тесты: не тупо запускай и закрывай проект, но:
  - Пользуйся всем доступным функционалом (нужным) MCP
  - Делай дебаг принты
  - Проси пользователя сделать какое-то конкретное действие и жди его
  - Если требуется тест пользователя - не закрывай проект раньше его ответа
  - Создавай @tool скрипты, и проси пользователя запускать их в редакторе
  - Читай консоль после тестов и делай выводы
  - После успешного теста, сразу удаляй все ненужные дебаги
